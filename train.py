...
# Custom MLP
generator = torch.nn.Sequential(MLPRepeat(dataset.item_count, dataset.item_count, 256, 1), torch.nn.Sigmoid())
discriminator = MLPRepeat(2*dataset.item_count, 1, 256, 1)

model = CFWGAN(train, test, None, dataset.item_count, alpha=0.1, s_zr=0.5, s_pm=0.5, d_steps=5, g_steps=1,
               config=config, custom_mlps=(generator, discriminator))
...
model = CFWGAN.load_from_checkpoint(model_checkpoint.best_model_path, trainset=train, valset=test, testset=None,
                                    num_items=dataset.item_count, custom_mlps=(generator, discriminator))
